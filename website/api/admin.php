...
try {
    switch ($action) {
        ...
        case 'dashboard':
            ...
            $row = $db->fetch("SELECT COUNT(*) as cnt FROM users WHERE role = 'user'");
            $stats['total_users'] = $row ? (int)$row['cnt'] : 0;

            $row = $db->fetch("SELECT COUNT(*) as cnt FROM users WHERE DATE(created_at) = CURDATE()");
            $stats['today_users'] = $row ? (int)$row['cnt'] : 0;

            $row = $db->fetch("SELECT COUNT(*) as cnt FROM users WHERE package_expires > NOW()");
            $stats['active_packages'] = $row ? (int)$row['cnt'] : 0;

            $row = $db->fetch("SELECT COUNT(*) as cnt FROM scans");
            $stats['total_scans'] = $row ? (int)$row['cnt'] : 0;

            $row = $db->fetch("SELECT COALESCE(SUM(asin_count), 0) as cnt FROM scans");
            $stats['total_asins'] = $row ? (int)$row['cnt'] : 0;

            $row = $db->fetch("SELECT COALESCE(SUM(amount), 0) as total FROM orders WHERE status = 'completed'");
            $stats['total_revenue'] = $row ? (float)$row['total'] : 0;

            $row = $db->fetch("SELECT COALESCE(SUM(amount), 0) as total FROM orders WHERE status = 'completed' AND MONTH(completed_at) = MONTH(CURDATE())");
            $stats['month_revenue'] = $row ? (float)$row['total'] : 0;
        ...
        case 'users':
            ...
            $row = $db->fetch("SELECT COUNT(*) as cnt FROM users $where", $params);
        ...
        case 'orders':
            ...
            $row = $db->fetch("SELECT COUNT(*) as cnt FROM orders");
            $total = $row ? (int)$row['cnt'] : 0;
        ...
        case 'settings':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                foreach ($settings as $s) {
                    $result[$s['setting_key']] = ['value' => $s['setting_value'], 'type' => $s['setting_type'] ?? 'text'];
                }
        ...
        case 'affiliate_earnings':
            ...
            $row = $db->fetch("SELECT COUNT(*) as cnt FROM affiliate_earnings ae $where", $params);
            $total = $row ? (int)$row['cnt'] : 0;
            ...
            $stats = $db->fetch(
                ...
                $statsParams
            ) ?: ['total_commission' => 0, 'total_orders' => 0, 'total_order_amount' => 0];
